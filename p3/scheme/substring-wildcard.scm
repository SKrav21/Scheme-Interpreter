...
;; I pretty much copied over my contains-substring code and changed one line. I will explain the change
(define (substring-wildcard source pattern)
    ...
    (define (substring-wildcard-helper source pattern i j) ;;define a helper function 
        (if (= j patternLen) #t ;;if this condition is satisfied, (1. continued below)
            (if (= i sourceLen) #f ;;means we have traversed the entire source and have not found the full pattern, (2. cont. below)
                (if (or (eq? (string-ref source i) (string-ref pattern j)) (eq? (string-ref pattern j) #\?)) ;;if char at i in source matches char j in pattern (6. cont below)
    ...
    (substring-wildcard-helper source pattern 0 0)) ;;initial call to helper function 
